...
export default function ChecklistCheck({ estabelecimentos, checklists, rts }: ChecklistCheckProps) {
  ...
  const formatSchedule = (day: string, value?: string) => {
    return (
      <div className="flex items-center justify-between text-xs border-b border-slate-100 py-2.5 last:border-0 hover:bg-white transition-colors px-1">
        <span className="font-bold text-slate-400 uppercase tracking-widest text-[9px] w-20">{day}</span>
        <span className="font-mono text-slate-700 font-bold text-[11px] select-all">{value || "FALTA ACORDO"}</span>
      </div>
    );
  };
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <span className="font-extrabold font-display text-[11px] truncate uppercase tracking-normal block max-w-[200px]">
                    {e.fantasia}
                  </span>
                  {isNovo && (
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 space-y-4">
              ...
              {!selectedChecklist ? (
                <div className="bg-slate-50 border border-slate-200 rounded-3xl p-8 text-center text-slate-400 text-xs shadow-inner">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3 bg-slate-50 border border-slate-200 p-3 rounded-2xl shadow-xs select-none">
                    <div className="text-center">
                      ...
                      <span className="text-sm font-black text-slate-800 font-mono mt-0.5 block">{selectedChecklist.totalAnaliseReceita}</span>
                    </div>
                    <div className="text-center">
                      <span className="text-[8px] text-slate-400 block font-bold uppercase tracking-wider">Esp. Portaria 344</span>
                      <span className="text-sm font-black text-slate-800 font-mono mt-0.5 block">{selectedChecklist.totalAnaliseNotificacao}</span>
                    </div>
                    <div className="text-center">
                  ...
                  <div className="border border-slate-200 rounded-2xl max-h-[300px] overflow-y-auto divide-y divide-slate-150 bg-slate-50/30 shadow-xs">
                    {QUESTAO_LIST.filter((q) => {
                      ...
                      return (
                        <div key={q.id} className="p-3.5 text-xs flex items-start gap-3 hover:bg-slate-50 transition-colors">
                          <span className="font-mono font-bold bg-white border border-slate-200 text-slate-500 px-2 py-0.5 rounded text-[9px] shrink-0 mt-0.5">
                            {q.id}
                          </span>
                          <p className={`flex-1 text-[11.5px] leading-relaxed ${answer === "S" ? "text-slate-600 font-medium" : "text-rose-700 font-extrabold"}`}>
                            {textToShow}
                          </p>
                          <div className={`shrink-0 flex items-center gap-1 font-extrabold text-[9px] px-2.5 py-0.5 rounded-full border ${
                            answer === "S" 
