...
const targetRegex = /className="w-full text-xs px-2\.5 py-1\.5 border border-slate-200 rounded-lg outline-none focus:border-violet-500"\s*\/>\s*<\/div>\s*<\/div>\s*<button\s*type="button"/;

const repl = `className="w-full text-xs px-2.5 py-1.5 border border-slate-200 rounded-lg outline-none focus:border-violet-500"
                                />
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col h-full bg-slate-50 border-l border-slate-100 items-center justify-center pl-2 ml-1">
                               <button
                                 type="button"`;

if (targetRegex.test(code)) {
   code = code.replace(targetRegex, repl);
   console.log("Success");
} else {
   console.log("Failed");
}
